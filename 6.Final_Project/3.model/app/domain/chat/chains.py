...
class RAGChain:
    ...
    def __init__(
        ...
        parent_retriever_factory: Callable[[str, str], BaseRetriever] | None = None,
    ):
        ...
        Args:
            ...
            parent_retriever_factory: ADR-076. (collection_type, user_id) -> BaseRetriever for resume/portfolio.
        """
        ...
        self._parent_retriever_factory = parent_retriever_factory
...
위 관련 정보를 참고하여 질문에 답변해주세요. 관련 정보가 없으면 일반적인 지식으로 답변해주세요.""",
    ...
    async def retrieve_context_documents(
        ...
        ADR-076: For resume/portfolio when rag_use_parent_retriever and parent_retriever_factory
        are set, uses parent document retriever instead of MMR.
        ...
        use_parent = (
            settings.rag_use_parent_retriever and self._parent_retriever_factory is not None
        )

        if self._vectorstores:
            for ct in collection_types:
                if use_parent and ct in ("resume", "portfolio"):
                    try:
                        retriever = self._parent_retriever_factory(ct, user_id)
                        docs = await asyncio.to_thread(retriever.invoke, query)
                        for doc in docs:
                            all_pairs.append((ct, doc))
                    except Exception as e:
                        logger.warning("Parent retriever failed for %s, fallback to MMR: %s", ct, e)
                        store = self._vectorstores.get(ct)
                        if store:
                            filter_dict = {"user_id": user_id} if user_id else None
                            pairs = await asyncio.to_thread(
                                self._mmr_search_one, ct, store, query, filter_dict
                            )
