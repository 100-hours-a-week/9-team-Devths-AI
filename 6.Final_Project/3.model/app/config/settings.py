...
class Settings(BaseSettings):
    ...
    # ADR-076: 부모 문서 리트리버 (이력서/포트폴리오 맥락 유지)
    rag_use_parent_retriever: bool = Field(
        default=True,
        description="Use ParentDocumentRetriever for resumes/portfolios (child 400 + parent 2000). ADR-076.",
    )
    rag_parent_chunk_size: int = Field(
        default=2000,
        description="Parent document chunk size (chars). ADR-076.",
    )
    rag_child_chunk_size: int = Field(
        default=400,
        description="Child chunk size for parent retriever (chars). ADR-076.",
    )
