...
class TextExtractRequest(BaseModel):
    ...
    # 문서 정보
    type: Literal["resume", "portfolio", "job_posting"] = Field(..., description="문서 타입")
    user_id: str = Field(..., min_length=1, description="사용자 ID", example="user_456")
    document_id: str = Field(..., min_length=1, description="문서 ID", example="resume_123")
    ...
    file_type: Literal["pdf", "image"] | None = Field(
        None, description="파일 타입 (file_url 사용 시 필수)"
    )
    text: str | None = Field(
        None, min_length=10, description="직접 입력한 텍스트 (file_url과 둘 중 하나 필수)"
    )
    ...
    def validate_file_url(cls, v):
        if v:
            ...
            # HTTP(S) URL 검증
                ...
                if not any(
                    ext in url_str.lower() for ext in [".pdf", ".png", ".jpg", ".jpeg", ".webp"]
                ):
    ...
    def validate_and_infer_file_type(cls, v, values):
        ...
        if not v:
            ...
            if url_str.startswith("data:image/"):
                return "image"
            elif url_str.startswith("data:application/pdf") or any(
                ext in url_str.lower() for ext in [".pdf"]
            ):
                return "pdf"
                ...
                raise ValueError(
                    "file_url 사용 시 file_type을 명시하거나 파일 확장자가 URL에 포함되어야 합니다"
                )
