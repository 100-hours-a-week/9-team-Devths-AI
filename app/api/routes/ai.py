...
from app.schemas.text_extract import TextExtractRequest, TextExtractResult, PageText
...
async def text_extract(request: TextExtractRequest):
    ...
    async def process_text_extract():
        try:
            ...
            logger.info(f"")
            logger.info(f"{'='*80}")
            logger.info(f"=== 📄 텍스트 추출 시작 (파일 업로드) ===")
            logger.info(f"{'='*80}")
            ...
            if request.file_url:
                ...
                if model == "vllm" and rag.vllm:
                    logger.info(f"💰 [vLLM 가성비 모드] pytesseract OCR 시작")
                ...
                # Gemini 모드: Gemini Vision API 사용 (고성능)
                    ...
                    logger.info(f"   → 고품질 OCR을 위해 Gemini Vision API 사용")
        ...
        try:
            ...
            if is_analysis:
                ...
                if model == "vllm" and rag.vllm:
                    ...
                    logger.info(f"   프로세스: pytesseract OCR → VectorDB 저장 → VectorDB 조회 → Llama 분석")
                    logger.info(f"")
...
async def calendar_parse(request: CalendarParseRequest):
    ...
    if not request.file_url and not request.text:
        raise HTTPException(
            ...
            detail={"code": ErrorCode.INVALID_REQUEST, "message": "file_url 또는 text 중 하나는 필수입니다."}
        )
